...
- [x] `/ads.txt`가 존재하되 승인 전 광고는 비활성인지 확인
...
- [x] `npm run lint`
- [x] `npm run build`
- [x] 실제 Vercel production URL에서 smoke test
- [x] Lighthouse 모바일 기준 사전 점검
...
This fix has been redeployed to Vercel production, and `/resources`, security headers, and PageSpeed console/a11y items have been verified live.

## 2026-05-09 Live Vercel smoke / PageSpeed result

Target: `https://aisurvivor.vercel.app`
...
- `/api/admin/me` response: `{ "admin": false }`. Anonymous public pages no longer create a console/network error.
- `/sitemap.xml`: includes `/resources`, excludes `/admin` and `/write`, 55 URLs total.
- `/rss.xml`: uses the Vercel production URL and does not contain `aivibelab.com`.
- `/robots.txt`: sitemap link and admin disallow are present.
- `/ads.txt`: placeholder remains until an AdSense publisher id exists.
- Live security headers are present: `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: DENY`, `Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=()`.

PageSpeed report: `icpca7ddqh`, 2026-05-09 15:48:29 KST

| Form factor | Performance | Accessibility | Best Practices | SEO |
| ----------- | ----------: | ------------: | -------------: | --: |
| Desktop     |          84 |           100 |            100 | 100 |
| Mobile      |          96 |           100 |            100 | 100 |

Interpretation:

- The previous console error and color contrast findings are fixed.
- Mobile meets the pre-launch threshold.
- Desktop Performance 84 is not a launch blocker. Keep it as optional backlog: main-thread work, TBT, unused JS, and render-blocking CSS.
